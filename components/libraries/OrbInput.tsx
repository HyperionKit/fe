...
export function OrbInput() {
  ...
  const placeholders = useMemo(
    () => [
      "Generate a DeFi Application",
      "Building Application",
      "How can I Build with Hyperkit",
  ...
  return (
    <div className="relative group">
      <div
        className={`flex items-center gap-2 p-3 bg-black transition-all duration-300 ease-out rounded-full border border-white hover:shadow-2xl hover:scale-105 hover:border-purple-400 ${
          isFocused ? "shadow-xl scale-[1.02] border-gray-300" : "shadow-lg"
        }`}
      >
        <div className="relative flex-shrink-0">
          <div className="w-8 h-8 rounded-full overflow-hidden transition-all duration-300 scale-100 group-hover:scale-110 group-hover:shadow-lg group-hover:shadow-purple-500/50">
            <img
        ...
        <div className="flex-1 w-[200px]">
          <input
            ...
            className="w-full text-sm text-white placeholder-gray-300 bg-transparent border-none outline-none font-light"
          />
        </div>
      </div>
    </div>
