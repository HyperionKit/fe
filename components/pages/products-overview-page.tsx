...
import OrbInput from '../visual/OrbInput';
import { Compare } from '../visual/Compare';
import ConnectWallet from '../defi/ConnectWallet';
import { useDemoWallet, useDemoTransaction, useDemoAuth } from '../demo/static/useDemoHooks';
...
export default function ProductsOverviewPage() {
  ...
  const [componentOrder, setComponentOrder] = useState<string[]>(['email', 'sms', 'social', 'passkey', 'externalWallets']);
  ...
  const handleDragOver = (e: React.DragEvent, componentKey: string) => {
    ...
    if (draggedItem && draggedItem !== componentKey) {
      const newOrder = [...componentOrder];
      const draggedIndex = newOrder.indexOf(draggedItem);
      const targetIndex = newOrder.indexOf(componentKey);
      ...
      <section className="mb-20 pt-8">
        ...
        <div className="flex flex-col lg:flex-row gap-10 items-start min-h-[900px]">
          ...
          <div className="flex-1 bg-white rounded-2xl shadow-2xl flex flex-col justify-center items-center p-6 relative h-[900px] overflow-hidden">
            ...
            {!codePreview && (Object.values(authSettings).filter(Boolean).length > 1) && (
...
{generateCodePreview()}
              ...
              <div className={`border border-gray-200 rounded-2xl shadow-xl w-[400px] min-h-[400px] max-h-[900px] flex flex-col items-center justify-start text-center px-6 py-6 transition-all duration-300 ${
                ...
                <div className="w-full max-w-sm">
                  ...
                  <div className={`w-full ${getDynamicSpacing()}`}>
                    {componentOrder.map((componentKey) => {
                      // Email Authentication
                      if (componentKey === 'email' && authSettings.email) {
                        const isDragging = draggedItem === 'email';
                        const isOver = dragOverItem === 'email';
                        ...
                        return (
                          ...
                          >
                            {/* Drop Zone Indicator */}
                            {isOver && draggedItem && draggedItem !== 'email' && (
                              <div className="absolute -top-2 left-0 right-0 h-1 bg-gradient-to-r from-purple-500 via-pink-500 to-purple-500 rounded-full animate-pulse shadow-lg shadow-purple-500/50"></div>
                            )}
                            
                            <div className={`space-y-3 rounded-2xl p-4 transition-all duration-300 border-2 ${
                              isDragging 
                                ? 'border-purple-500 bg-purple-50 shadow-2xl shadow-purple-500/50 cursor-grabbing' 
                                : isOver
                                ? 'border-purple-300 bg-purple-50/50 scale-105'
                                : 'border-transparent hover:border-purple-200 hover:bg-purple-50/30 cursor-grab hover:shadow-lg'
                            } ${brandingSettings.theme === 'dark' ? (isDragging ? 'bg-purple-900/20' : isOver ? 'bg-purple-900/10' : 'hover:bg-gray-800/30') : ''}`}>
                              ...
                              <div className={`absolute -left-3 top-1/2 -translate-y-1/2 transition-all duration-300 ${
                                ...
                                <div className="bg-gradient-to-br from-purple-600 to-indigo-600 text-white p-2 rounded-lg shadow-xl border-2 border-white/30">
                              ...
                              {/* Drag Status Badge */}
                              {isDragging && (
                                <div className="absolute -top-3 left-1/2 -translate-x-1/2 bg-gradient-to-r from-purple-600 to-pink-600 text-white px-3 py-1 rounded-full text-xs font-bold shadow-lg animate-bounce">
                                  Moving...
                                </div>
                              )}
                      ...
                      // SMS Authentication
                      if (componentKey === 'sms' && authSettings.sms) {
                        const isDragging = draggedItem === 'sms';
                        const isOver = dragOverItem === 'sms';
                        ...
                        return (
                          <div 
                            ...
                            onDragOver={(e) => handleDragOver(e, 'sms')}
                            onDragLeave={handleDragLeave}
                            onDragEnd={handleDragEnd}
                            className={`group relative transition-all duration-300 ease-out ${
                              isDragging 
                          ...
                          >
                            {isOver && draggedItem && draggedItem !== 'sms' && (
                              <div className="absolute -top-2 left-0 right-0 h-1 bg-gradient-to-r from-purple-500 via-pink-500 to-purple-500 rounded-full animate-pulse shadow-lg shadow-purple-500/50"></div>
                            )}
                      ...
                      if (componentKey === 'social' && authSettings.social) {
                        const isDragging = draggedItem === 'social';
                        const isOver = dragOverItem === 'social';
                        ...
                        return (
                          ...
                          >
                            {isOver && draggedItem && draggedItem !== 'social' && (
                              <div className="absolute -top-2 left-0 right-0 h-1 bg-gradient-to-r from-purple-500 via-pink-500 to-purple-500 rounded-full animate-pulse shadow-lg shadow-purple-500/50"></div>
                            )}
                      ...
                      if (componentKey === 'passkey' && authSettings.passkey) {
                        const isDragging = draggedItem === 'passkey';
                        const isOver = dragOverItem === 'passkey';
                        
                        return (
                          <div 
                            key="passkey"
                            draggable
                            onDragStart={(e) => handleDragStart(e, 'passkey')}
                            onDragOver={(e) => handleDragOver(e, 'passkey')}
                            onDragLeave={handleDragLeave}
                            onDragEnd={handleDragEnd}
                            className={`group relative transition-all duration-300 ease-out ${
                              isDragging 
                          ...
                          >
                            {isOver && draggedItem && draggedItem !== 'passkey' && (
                              <div className="absolute -top-2 left-0 right-0 h-1 bg-gradient-to-r from-purple-500 via-pink-500 to-purple-500 rounded-full animate-pulse shadow-lg shadow-purple-500/50"></div>
                            )}
                      ...
                      // External Wallets
                      if (componentKey === 'externalWallets' && authSettings.externalWallets) {
                        const isDragging = draggedItem === 'externalWallets';
                        const isOver = dragOverItem === 'externalWallets';
                        
                        return (
                          <div 
                            key="externalWallets"
                            draggable
                            onDragStart={(e) => handleDragStart(e, 'externalWallets')}
                            onDragOver={(e) => handleDragOver(e, 'externalWallets')}
                            onDragLeave={handleDragLeave}
                            onDragEnd={handleDragEnd}
                            className={`group relative transition-all duration-300 ease-out ${
                              isDragging 
                          ...
                          >
                            {isOver && draggedItem && draggedItem !== 'externalWallets' && (
                              <div className="absolute -top-2 left-0 right-0 h-1 bg-gradient-to-r from-purple-500 via-pink-500 to-purple-500 rounded-full animate-pulse shadow-lg shadow-purple-500/50"></div>
                            )}
